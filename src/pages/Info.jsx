...
const Info = () => {
  ...
  const fetchComputers = async () => {
    ...
    try {
      ...
      const computerResponse = await fetch(
        `https://mundojob.with23.glpi-network.cloud/apirest.php/Computer/?range=0-500&expand_dropdowns=true`,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchPhones = async () => {
    try {
      // Solicitar activos utilizando el token de sesión
      const computerResponse = await fetch(
        "https://mundojob.with23.glpi-network.cloud/apirest.php/Phone/?range=0-500&expand_dropdowns=true",
        {
          headers: {
            "Content-Type": "application/json",
            "Session-Token": sessionToken,
  ...
  const fetchMonitors = async () => {
    try {
      // Solicitar activos utilizando el token de sesión
      const computerResponse = await fetch(
        "https://mundojob.with23.glpi-network.cloud/apirest.php/Monitor/?range=0-500&expand_dropdowns=true",
        {
          headers: {
            "Content-Type": "application/json",
            "Session-Token": sessionToken,
